...
Peaks were annotated using corresponding R packages EnsDb.Hsapiens.v79 and TxDb.Hsapiens.UCSC.hg38.knownGene by ChIPseeker (1.14.2).<br>
...
ref_fa='/project/bhimeslab/Reference/hg38/genome.ERCC.fa'
...
if('Input' %in% colnames(coldata)){bamreads=coldata$bamReads[!is.na(coldata$Input)]
} else {bamreads=coldata$bamReads}
...
if('Input' %in% colnames(coldata)){coldata$bamControl=paste0(path_start,coldata$Input,'/bwa_out/',coldata$Input,'.bam')
...
if('Input' %in% colnames(coldata)){bedpeaks=coldata$Peaks[!is.na(coldata$Input)]
} else {bedpeaks=coldata$Peaks}
