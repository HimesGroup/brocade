...
mapped.percent.for.plot <- rbind(
  data.frame(
    ...
    Type=rep("Mapped",nrow(bamstats.percent.table))),
  data.frame(
    variable=colnames(bamstats.percent.table),
    value=as.numeric(bamstats.percent.table["Unmapped reads",]),
    Type=rep("Unmapped",nrow(bamstats.percent.table)))
)
...
Perform PCA using read scores from DiffBind. DiffBind will compute read counts for all samples. This process takes a while.
